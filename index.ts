...
function carDetails<T>(
    manufacturer: string,
    modelName: string,
    ...additionalInfo: [string, T][] // Rest parameter with generic type T
  ) {
    const car = { manufacturer, modelName, ...Object.fromEntries(additionalInfo) };
    return car;
  }
  
  const myCarDetails = carDetails<string>("Toyota", "Corolla", ['color', 'blue'], ['year', "2022"]); // Specify type for additionalInfo elements
