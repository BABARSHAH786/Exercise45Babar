...
function carDetails(manufacturer, modelName) {
    var additionalInfo = []; // Rest parameter with generic type T
    for (var _i = 2 // Rest parameter with generic type T
    ; _i < arguments.length // Rest parameter with generic type T
    ; _i++ // Rest parameter with generic type T
    ) {
        additionalInfo[_i - 2] = arguments[_i]; // Rest parameter with generic type T
    }
    var car = __assign({ manufacturer: manufacturer, modelName: modelName }, Object.fromEntries(additionalInfo));
    return car;
}
var myCarDetails = carDetails("Toyota", "Corolla", ['color', 'blue'], ['year', "2022"]); // Specify type for additionalInfo elements
console.log(myCarDetails);
